...
1. **통신 방법 변경** 
    - 문제 : 
      ...
      초기에는 통신 방법을 다음과 같이 설정했습니다.
      - main -> client : 메시지 큐
      - client <-> client : 메시지 큐
      - client -> server : 공유 메모리
      
      그러나 해당 방법은 클라이언트간 데이터 소통이 매우 비효율적이고, 로직이 어려워지는 문제가 발생하였습니다. 

      해당 문제들은 다음과 같습니다.

      1. **N:N 통신의 지옥** : 0번 클라이언트가 가진 데이터 중에 1번 거, 2번 거, ... 7번 거가 섞여 있습니다. 즉, 모든 클라이언트가 모든 클라이언트에게 데이터를 쪼개서 보내야 합니다.

      2. **오버헤드 폭발** : 메시지 큐는 커널을 거쳐 복사가 일어납니다. 서로 흩어진 데이터를 맞교환하기 위해 수십~수백 번의 msgsnd/msgrcv가 발생하며, 이는 성능 저하와 데드락(Deadlock)의 위험을 높입니다.

      3. **복잡한 동기화** : "내가 1~7번 클라이언트로부터 모든 데이터를 다 받았나?"를 확인하는 로직이 매우 복잡해집니다.
    
    <br>

    - 해결 : 
    
      이 해당 문제를 해결하고자 통신 방법을 다음과 같이 바꾸는 것이 제안되었습니다.
      - main -> client : 메시지 큐
      - client <-> client : 공유 메모리
      - client -> server : 메시지 큐

      위와 같이 통신 방법을 변경함으로서 클라이언트간 통신 로직을 매우 간단하고 효율적이게 변경할 수 있었습니다.

      해당 방법을 검토 결과 통신 방법을 바꿔 얻는 이득이 유지함으로서 얻는 이득보다 크다고 생각하여 통신방법을 변경하기로 결정했고 무탈히 잘 해결되었습니다.
...
* **하이브리드 IPC 통신:**  
  ...
  * **Client → Server:** 메시지 큐(Message Queue)를 사용하여 비동기 데이터 전송.   
...
├── server.h           \# 클라이언트 로직 헤더  
├── server.c           \# 클라이언트 로직 구현 (MQ 수신 및 SHM 정렬)  
